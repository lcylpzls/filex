...
func init() {
	...
	register(CodeUnauthorized, "未认证", errx.KindUnauthorized)
	register(CodeForbidden, "无权限", errx.KindForbidden)
	register(CodeNotModified, "对象未修改", errx.KindConflict)
	register(CodePreconditionFailed, "前置条件不满足", errx.KindConflict)
	register(CodeCancelled, "操作已取消", errx.KindCancelled)
	register(CodeClosed, "存储已关闭", errx.KindUnavailable)
