...
> 状态：**已发布**（v0.5.0，2026-08-10）。静态加密、Bearer/HMAC/回调
> 鉴权、防重放与审计全部落地，四包覆盖率均 100%。
...
> 状态：**已发布**（v0.6.0，2026-08-10）。生命周期、孤儿巡检、健康检查、
> 请求指标与基准全部落地，四包覆盖率均 100%。
...
- v0.8.0：孤儿巡检保护活动分片会话；
- v0.9.0 / v0.9.1：条件请求错误码明确化与格式修正；
- v0.10.0：版本 ID 响应头、终审文档与 API 冻结快照；
- v0.11.x：完整性审计、公开桶用量、上下文取消语义；
- v0.12.0：分片会话 TTL、DeleteBucket 活动上传保护、CI 工业化；
- v0.13.0：客户端 Content-Length、并发基准。
- v0.14.0：BucketStats、运维手册、版本化迁移兼容修复。
- v0.15.0：桶元数据 fuzz、API 快照更新。
- v0.16.0：Store 关闭态、生命周期/巡检独占锁、GET Bucket。
- v0.17.0：分块 AES-256-GCM 认证加密升级。
- v0.18.0：长任务取消贯穿、Release 乱序测试。

> 状态：**1.0 候选**（v0.18.0，2026-08-10）。
> 终审见 [final-review.md](final-review.md)。
