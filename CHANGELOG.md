...
- `TraceHook` / `TraceAttr` 契约迁移至零依赖子模块
  `tracex/contract`（v1.0.0），filex 不再依赖 tracex 主模块。
...
- 白盒测试迁入 `internal/core`，根包新增黑盒冒烟测试，覆盖率 100%；
- 公开子包 client/proto/server 保持不变。
...
- Metrics 契约统一为 `metricsx.Sink`（破坏性：注入实现需满足 Sink 协议，字节/错误语义映射为计数指标）；
- Trace 契约统一为 `tracex.TraceHook`（类型别名，调用点不变）；
- 依赖升级：tracex v1.0.4、metricsx v1.5.4、errx v1.5.7、logx v1.3.4、cryptox v1.0.4、idgenx v1.4.4、validx v1.2.5、testx v1.4.5。
...
- 示例命令行接入 clix：protocol / http3 示例由 stdlib flag 迁移到
  `clix v1.2.0`（全局 flag、必填校验、统一帮助与退出码）；
- 示例模块新增 clix 依赖，confx/logx/validx 同步升级。

### 质量

- 示例端到端测试（协议 / HTTP/3）全绿；race / vet / staticcheck 通过。
